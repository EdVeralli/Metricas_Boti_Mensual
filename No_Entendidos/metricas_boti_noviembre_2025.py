...
def calcular_promedios_boti():
    try:
        ...
        # PASO 6: LIMPIEZA
        # =================================================================
        imprimir_seccion("PASO 6: LIMPIEZA")
        
        mm1.reset_index(inplace=True, drop=True)
        
        drop = []
        for i in mm1.index[:-1]:
            if (mm1.loc[i].msg_from == mm1.loc[i+1].msg_from and
                mm1.loc[i].session_id == mm1.loc[i+1].session_id):
                drop.append(i)
        
        if drop:
            mm1.drop(drop, inplace=True)
            mm1.reset_index(inplace=True, drop=True)
            imprimir_progreso(f"✓ Eliminados: {len(drop):,}")
