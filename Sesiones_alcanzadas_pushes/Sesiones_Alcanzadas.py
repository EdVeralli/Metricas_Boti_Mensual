...
Script para ejecutar query de Sesiones Alcanzadas por Pushes en Athena
Guarda resultado en CSV y escribe el resultado en la celda D5 de Excel
...
def build_query(fecha_inicio, fecha_fin):
    """Construye la query de Sesiones Alcanzadas con el rango de fechas especificado"""
    
    query = """SELECT count(distinct session_id) as count_sessions
...
AND regexp_like(message, '^Template')""".format(fecha_inicio=fecha_inicio, fecha_fin=fecha_fin)
...
def generate_filename(modo, mes, anio, fecha_inicio, fecha_fin):
    ...
    if modo == 'mes':
        mes_nombre = get_month_name(mes)
        filename_csv = "sesiones_alcanzadas_{0}_{1}.csv".format(mes_nombre, anio)
        filename_excel = "sesiones_alcanzadas_{0}_{1}.xlsx".format(mes_nombre, anio)
    else:
        fecha_inicio_fmt = fecha_inicio.replace('-', '')
        fecha_fin_fmt = fecha_fin.replace('-', '')
        filename_csv = "sesiones_alcanzadas_{0}_a_{1}.csv".format(fecha_inicio_fmt, fecha_fin_fmt)
        filename_excel = "sesiones_alcanzadas_{0}_a_{1}.xlsx".format(fecha_inicio_fmt, fecha_fin_fmt)
...
def create_excel_with_dashboard(filepath, result_value, modo, mes, anio, fecha_inicio, fecha_fin):
    ...
    Escribe el resultado SOLO en la celda D5 (Sesiones Alcanzadas por Pushes)
    """
    ...
    # FILA 5: RESULTADO - Sesiones Alcanzadas por Pushes
    ws['B5'] = 'Sesiones Alcanzadas por Pushes'
    ws['C5'] = 'Q Sesiones que recibieron al menos 1 Push'
    ws['D5'] = result_value  # ← UNICO VALOR QUE SE ESCRIBE
...
def execute_query_and_save():
    ...
    try:
        ...
        if len(df) > 0 and 'count_sessions' in df.columns:
            result_value = int(df['count_sessions'].iloc[0])
        else:
        ...
        print("=" * 60)
        print("Sesiones Alcanzadas por Pushes: {:,}".format(result_value))
        print("=" * 60)
        ...
        print("            Celda D5 = {:,}".format(result_value))
...
if __name__ == "__main__":
    print("")
    print("=" * 60)
    print("SCRIPT: SESIONES ALCANZADAS POR PUSHES - QUERY ATHENA V2")
    print("=" * 60)
    print("Rol requerido: PIBAConsumeBoti")
    print("Salida: CSV + Excel Dashboard (celda D5)")
    print("")
    print("MODOS:")
