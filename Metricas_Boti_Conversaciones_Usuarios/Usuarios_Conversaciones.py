...
Script para ejecutar query de Usuarios y Conversaciones en Athena
Guarda resultado en CSV y escribe los resultados en las celdas D2 y D3 de Excel
...
def build_query(fecha_inicio, fecha_fin):
    """Construye la query de Usuarios y Conversaciones con el rango de fechas especificado"""
...
WHERE CAST(session_creation_time AS DATE) BETWEEN date '{fecha_inicio}' and date '{fecha_fin}'""".format(fecha_inicio=fecha_inicio, fecha_fin=fecha_fin)
...
def generate_filename(modo, mes, anio, fecha_inicio, fecha_fin):
    ...
    if modo == 'mes':
        mes_nombre = get_month_name(mes)
        filename_csv = "usuarios_conversaciones_{0}_{1}.csv".format(mes_nombre, anio)
        filename_excel = "usuarios_conversaciones_{0}_{1}.xlsx".format(mes_nombre, anio)
    else:
        fecha_inicio_fmt = fecha_inicio.replace('-', '')
        fecha_fin_fmt = fecha_fin.replace('-', '')
        filename_csv = "usuarios_conversaciones_{0}_a_{1}.csv".format(fecha_inicio_fmt, fecha_fin_fmt)
        filename_excel = "usuarios_conversaciones_{0}_a_{1}.xlsx".format(fecha_inicio_fmt, fecha_fin_fmt)
    
    return filename_csv, filename_excel

def create_excel_with_dashboard(filepath, cant_conversaciones, cant_usuarios, modo, mes, anio, fecha_inicio, fecha_fin):
    """
    Crea un Excel NUEVO desde cero con estructura de Dashboard completa
    Escribe los resultados en las celdas D2 (Conversaciones) y D3 (Usuarios)
    """
    ...
    # FILA 2: Conversaciones - AQUI VA EL RESULTADO
    ws['B2'] = 'Conversaciones'
    ws['C2'] = 'Q Conversaciones'
    ws['D2'] = cant_conversaciones  # ← VALOR DE CANT_SESIONES
    
    # FILA 3: Usuarios - AQUI VA EL RESULTADO
    ws['B3'] = 'Usuarios'
    ws['C3'] = 'Q Usuarios únicos'
    ws['D3'] = cant_usuarios  # ← VALOR DE CANT_USUARIO
...
def execute_query_and_save():
    ...
    try:
        ...
        if len(df) > 0 and 'Cant_Usuario' in df.columns and 'Cant_Sesiones' in df.columns:
            cant_usuarios = int(df['Cant_Usuario'].iloc[0])
            cant_conversaciones = int(df['Cant_Sesiones'].iloc[0])
        else:
            print("[ERROR] No se pudo obtener el resultado")
            print("    Columnas disponibles: {}".format(list(df.columns) if len(df) > 0 else 'Sin datos'))
            return None
        ...
        print("=" * 60)
        print("Usuarios (D3): {:,}".format(cant_usuarios))
        print("Conversaciones (D2): {:,}".format(cant_conversaciones))
        print("=" * 60)
        ...
        create_excel_with_dashboard(local_path_excel, cant_conversaciones, cant_usuarios, 
        ...
        print("            Celda D2 (Conversaciones) = {:,}".format(cant_conversaciones))
        print("            Celda D3 (Usuarios) = {:,}".format(cant_usuarios))
